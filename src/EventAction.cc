...
#include "EventAction.hh"		// Specifies the file which contains the class structure

#include "EventMessenger.hh"		// Specifies user-defined classes which are called upon in this class
...
#include "G4ios.hh"			// Specifies the classes which allow reading/writing from standard input/output
#include <iomanip>
#include <iostream>
#include <fstream>
...
void EventAction::EndOfEventAction(const G4Event* anEvent)
{
  ...
  if(InnerNHitHCE && InnerNHitHCE->entries())
    {
      ...
      for(int i = 0; i < nCountInner; i++)		// This loop used to record neutron statistics
	{
	  ...
	  if((*InnerNHitHCE)[i]->GetCaptured())	// Determines if neutron was captured
	    {
	  ...
	  if((*InnerNHitHCE)[i]->GetForeign())	// Determines if neutron entered into sensitive volume from external source
	    {
  ...
  if(PMTPhotHitHCE && PMTPhotHitHCE->entries())
      ...
      for(int i = 0; i < n; i++)			// This loop activates visualization process for photon sensitive volumes (code located in OpticalHit.cc)
	{
      ...
      for(int i = 0; i < n; i++)			// This loop used to record photon statistics
	{
  ...
  if(InnerPHitHCE && InnerPHitHCE->entries())
    {
      ...
      for(int i = 0; i < pCountInner; i++)		// This loop used to record proton statistics
	{
  ...
  if(InnerEDepHCE && InnerEDepHCE->entries())	// Scintillator Volume
    {
  ...
  if(EventTime) { EventTime->clear(); }		// Clears the histograms for next event - possibly a redundant feature, but saves memory
...
void EventAction::SetPrintPerNEvents(G4int num)
{
  ...
  { G4cout << "*** CAUTION: Simulation will no longer print data at event checkpoints. All data will be printed at the end of run. ***" << G4endl; fPrintPerEvent = 0; }
  else
  {
    fPrintPerEvent = num;
