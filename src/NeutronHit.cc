...
G4Allocator<NeutronHit> NeutronHitAllocator;	// Creates an allocator for the NeutronHit class - allocates memory according to size of data to be stored
...
const NeutronHit& NeutronHit::operator=(const NeutronHit &right)	// Assignment operator function
{
  positionI = right.positionI;
  positionF = right.positionF;
  energy = right.energy;
...
int NeutronHit::operator==(const NeutronHit &right) const		// Equality operator function
{
  return (trackID==right.trackID);
}

void NeutronHit::operator+=(const NeutronHit &right)			// Increment operator function
{
  positionF = right.positionF;
  time = right.time;
...
void NeutronHit::Draw()
{
  ...
  if(vis_manager)
  {
    if(foreign)				// Entry Position
    {
      // Create a circle to identify hit location

      G4Point3D* pointI = new G4Point3D(positionI);		// There's some odd pointer work in here, good for understanding how pointers work
      G4Circle* circleI = new G4Circle(*pointI);
      circleI->SetScreenSize(5);

      // Make circle yellow

      G4VisAttributes* attributesI = new G4VisAttributes();
    ...
    if(captured)			// Capture Position
    {
      // Create a circle to identify hit location

      G4Point3D* pointF = new G4Point3D(positionF);		// There's some odd pointer work in here, good for understanding how pointers work
      G4Circle* circleF = new G4Circle(*pointF);
      circleF->SetScreenSize(5);

      // Make circle red

      G4VisAttributes* attributesF = new G4VisAttributes();
...
std::vector<G4AttValue>* NeutronHit::CreateAttValues() const
{
  ...
  values->push_back(G4AttValue("Gamma Count",gamma,""));
