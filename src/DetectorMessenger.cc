...
#include "globals.hh"				// Specifies class defining all global parameters and variable types
	...
	// ****** Constructor ****** //
DetectorMessenger::DetectorMessenger(DetectorConstruction* det_const)
  :  detector(det_const)		// Allows retrieval of DetectorConstruction to set user-defined values
{
  ...
  segLCmd = new G4UIcmdWithADouble("/detector/setSegmentLength",this);
  segLCmd->SetGuidance("Set the length(mm) of a segment (does not include PMT length)");
  segLCmd->SetParameterName("SegL",false);
  segLCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);
  
  segWCmd = new G4UIcmdWithADouble("/detector/setSegmentWidth",this);
  segWCmd->SetGuidance("Set the width(mm) of a segment");
  segWCmd->SetParameterName("SegW",false);
  segWCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);
  
  segHCmd = new G4UIcmdWithADouble("/detector/setSegmentHeight",this);
  segHCmd->SetGuidance("Set the height(mm) of a segment");
  segHCmd->SetParameterName("SegH",false);
  segHCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);

  shLeadCmd = new G4UIcmdWithADouble("/detector/setLeadShieldThickness",this);
  shLeadCmd->SetGuidance("Set the thickness(mm) of the lead shield");
  shLeadCmd->SetParameterName("ShLead",false);
  shLeadCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);

  shBPolyCmd = new G4UIcmdWithADouble("/detector/setBPolyShieldThickness",this);
  shBPolyCmd->SetGuidance("Set the thickness(mm) of the borated poly shield");
  shBPolyCmd->SetParameterName("ShBPoly",false);
  shBPolyCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);

  shLiPolyCmd = new G4UIcmdWithADouble("/detector/setLiPolyShieldThickness",this);
  shLiPolyCmd->SetGuidance("Set the thickness(mm) of the lithiated poly shield");
  shLiPolyCmd->SetParameterName("ShLiPoly",false);
  shLiPolyCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);

  scinLCmd = new G4UIcmdWithADouble("/detector/setScintillatorLength",this);
  scinLCmd->SetGuidance("Set the length(mm) of the scintillator");
  scinLCmd->SetParameterName("ScinL",false);
  scinLCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);
  
  scinWCmd = new G4UIcmdWithADouble("/detector/setScintillatorWidth",this);
  scinWCmd->SetGuidance("Set the width(mm) of the scintillator");
  scinWCmd->SetParameterName("ScinW",false);
  scinWCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);
  
  scinHCmd = new G4UIcmdWithADouble("/detector/setScintillatorHeight",this);
  scinHCmd->SetGuidance("Set the height(mm) of the scintillator");
  scinHCmd->SetParameterName("ScinH",false);
  scinHCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);
  
  segBuffCmd = new G4UIcmdWithADouble("/detector/setSegmentBuffer",this);
  segBuffCmd->SetGuidance("Set the buffer distance(mm) between the PMTs and scintilator");
  segBuffCmd->SetParameterName("SegBuff",false);
  segBuffCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);

  segAirCmd = new G4UIcmdWithADouble("/detector/setAirGap",this);
  segAirCmd->SetGuidance("Set the air gap(mm) between segments");
  segAirCmd->SetParameterName("SegAir",false);
  segAirCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);

  segWrapGapCmd = new G4UIcmdWithADouble("/detector/setWrapGap",this);
  segWrapGapCmd->SetGuidance("Set the air gap(mm) between segments");
  ...
  segWrapCmd = new G4UIcmdWithADouble("/detector/setWrapThickness",this);
  segWrapCmd->SetGuidance("set the thickness(mm) of the outer segment wrap (diffuse reflector)");
  segWrapCmd->SetParameterName("SegWrap",false);
  segWrapCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);

  segAcrylCmd = new G4UIcmdWithADouble("/detector/setAcrylThickness",this);
  segAcrylCmd->SetGuidance("set the thickness(mm) of the segment acrylic (excludes buffer dimension)");
  segAcrylCmd->SetParameterName("SegAcryl",false);
  segAcrylCmd->AvailableForStates(G4State_PreInit,G4State_Init,G4State_Idle);

  segPMTCmd = new G4UIcmdWithADouble("/detector/setPMTDiameter",this);
  segPMTCmd->SetGuidance("set the diameter of the PMT (in)");
...
void DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  {
    detector->SetScintillatorBirksConstant(birksCmd->GetNewDoubleValue(newValue)*mm/MeV);
  }
  else if(command == spikeCmd)
  {
    detector->SetSpecularSpikeConstant(spikeCmd->GetNewDoubleValue(newValue));
  }
  else if(command == effCmd)
  {
    detector->SetEfficiency(effCmd->GetNewDoubleValue(newValue));
  }
  else if(command == backCmd)
  {
    detector->SetBackScatterConstant(backCmd->GetNewDoubleValue(newValue));
  }
  else if(command == QECmd)
    {
      detector->SetQE(QECmd->GetNewDoubleValue(newValue));
    }
   else if(command == segXCmd)
  {
    detector->SetXSegments(segXCmd->GetNewIntValue(newValue));
  }
   else if(command == segYCmd)
  {
    detector->SetYSegments(segYCmd->GetNewIntValue(newValue));
  }
  else if(command == segLCmd)
  {
    detector->SetSegmentLength(segLCmd->GetNewDoubleValue(newValue));
  }
  else if(command == segWCmd)
  {
    detector->SetSegmentWidth(segWCmd->GetNewDoubleValue(newValue));
  }
  else if(command == segHCmd)
  {
    detector->SetSegmentHeight(segHCmd->GetNewDoubleValue(newValue));
  }
  else if(command == shLeadCmd)
  {
    detector->SetLeadShieldThickness(shLeadCmd->GetNewDoubleValue(newValue));
  }
  else if(command == shBPolyCmd)
  {
    detector->SetBPolyShieldThickness(shBPolyCmd->GetNewDoubleValue(newValue));
  }
  else if(command == shLiPolyCmd)
  {
    detector->SetLiPolyShieldThickness(shLiPolyCmd->GetNewDoubleValue(newValue));
  }
  else if(command == scinLCmd)
  {
    detector->SetScintLength(scinLCmd->GetNewDoubleValue(newValue));
  }
  else if(command == scinWCmd)
  {
    detector->SetScintWidth(scinWCmd->GetNewDoubleValue(newValue));
  }
  else if(command == scinHCmd)
  {
    detector->SetScintHeight(scinHCmd->GetNewDoubleValue(newValue));
  }
   else if(command == segBuffCmd)
  {
    detector->SetSegmentBuffer(segBuffCmd->GetNewDoubleValue(newValue));
  }
  else if(command == segAirCmd)
  {
    detector->SetAirGap(segAirCmd->GetNewDoubleValue(newValue));
  }
  else if(command == segWrapGapCmd)
  {
    detector->SetWrapGap(segWrapGapCmd->GetNewDoubleValue(newValue));
  }
  else if(command == segWrapCmd)
  {
    detector->SetWrapThickness(segWrapCmd->GetNewDoubleValue(newValue));
  }
  else if(command == segAcrylCmd)
  {
    detector->SetAcrylThickness(segAcrylCmd->GetNewDoubleValue(newValue));
  }
 else if(command == segPMTCmd)
  {
    detector->SetPMTDiameter(segPMTCmd->GetNewDoubleValue(newValue));
  }
  else if(command == lobeCmd)
  {
    detector->SetSpecularLobeConstant(lobeCmd->GetNewDoubleValue(newValue));
  }
  else if(command == sigalCmd)
  {
    detector->SetSigmaAlpha(sigalCmd->GetNewDoubleValue(newValue));
  }
  else if(command == reflCmd)
  {
    detector->SetReflectivityConstant(reflCmd->GetNewDoubleValue(newValue));
  }
  else if(command == scintCompCmd)
  {
    detector->SetScintillatorComposition(newValue);
  }
 else if(command == vertCmd)
  {
    detector->SetVertical(vertCmd->GetNewBoolValue(newValue));

  }
 else if(command == modCmd)
  {
    detector->Modify();

  }
 else if(command == conCmd)
  {
    detector->ConstructNew();

  }
  else if(command == innerCmd)
  {
    detector->SetInnerTankGeometry(innerCmd->GetNewBoolValue(newValue));

  }
  else if(command == outerCmd)
  {
    detector->SetOuterTankGeometry(outerCmd->GetNewBoolValue(newValue));

  }
  else if(command == polyCmd)
    {
      detector->SetBoratedPolyLayer(polyCmd->GetNewBoolValue(newValue));
  ...
  else if(command == shieldCmd)
    {
      detector->SetShieldLayer(shieldCmd->GetNewBoolValue(newValue));

    }
  else if(command == cylinderCmd)
    {
      detector->SetCylinder(cylinderCmd->GetNewBoolValue(newValue));

    }
  else if(command == vetoCmd)
    {
      detector->SetMuonVetoLayer(vetoCmd->GetNewBoolValue(newValue));
  ...
  else if(command == explodeCmd)
  {
    detector->SetExplodedView(explodeCmd->GetNewBoolValue(newValue));

  }
  else if(command == printCmd)
  {
    detector->PrintPhysicalVolumes();
  }
  else
  {
