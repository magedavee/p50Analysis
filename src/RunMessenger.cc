...
void RunMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if(command == outNumCmd)
  ...
  else if(command == outRawCmd)
    {
      ...
      if(output == "nCapture")
	{
	  run->SetNeutronCaptureRawDataOutput(condition);
	}
      else if(output == "opPhoton")
	{
	  run->SetOpticalPhotonRawDataOutput(condition);
	}
      else if(output == "opLiquid")
	{
	  run->SetLSPhotonCountRawDataOutput(condition);
	}
      else if(output == "opVeto")
	{
	  run->SetVetoPanelCountRawDataOutput(condition);
	}
    }
  ...
  else if(command == nEnRefCmd)
    {
      run->SetNeutronCaptureEnergyLowerRef(nEnRefCmd->GetNewDoubleValue(newValue));
    }
  else if(command == nEnNumCmd)
    {
      run->SetNeutronCaptureEnergyBinCount(nEnNumCmd->GetNewIntValue(newValue));
    }
  else if(command == nTmBinCmd)
  {
    run->SetNeutronCaptureTimeBinWidth(nTmBinCmd->GetNewDoubleValue(newValue));
  }
  else if(command == nTmRefCmd)
  {
    run->SetNeutronCaptureTimeLowerRef(nTmRefCmd->GetNewDoubleValue(newValue));
  }
  else if(command == nTmNumCmd)
  {
    run->SetNeutronCaptureTimeBinCount(nTmNumCmd->GetNewIntValue(newValue));
  }
  else if(command == nThBinCmd)
  {
    run->SetNeutronThermalizationTimeBinWidth(nThBinCmd->GetNewDoubleValue(newValue));
  }
  else if(command == nThRefCmd)
  {
    run->SetNeutronThermalizationTimeLowerRef(nThRefCmd->GetNewDoubleValue(newValue));
  }
  else if(command == nThNumCmd)
  {
    run->SetNeutronThermalizationTimeBinCount(nThNumCmd->GetNewIntValue(newValue));
  }
  else if(command == nDsBinCmd)
  {
    run->SetNeutronCaptureDistanceBinWidth(nDsBinCmd->GetNewDoubleValue(newValue));
  }
  else if(command == nDsRefCmd)
  {
    run->SetNeutronCaptureDistanceLowerRef(nDsBinCmd->GetNewDoubleValue(newValue));
  }
  else if(command == nDsNumCmd)
  {
    run->SetNeutronCaptureDistanceBinCount(nDsNumCmd->GetNewIntValue(newValue));
  }
  else if(command == pEnBinCmd)
  {
    run->SetRecoilProtonEnergyBinWidth(pEnBinCmd->GetNewDoubleValue(newValue));
  }
  else if(command == pEnRefCmd)
  {
    run->SetRecoilProtonEnergyLowerRef(pEnRefCmd->GetNewDoubleValue(newValue));
  }
  else if(command == pEnNumCmd)
  {
    run->SetRecoilProtonEnergyBinCount(pEnNumCmd->GetNewIntValue(newValue));
  }
  ...
  else if(command == nEERefCmd)
  {
    run->SetNeutronEntryEnergyLowerRef(nEERefCmd->GetNewDoubleValue(newValue));
  }
  else if(command == nEENumCmd)
  {
    run->SetNeutronEntryEnergyBinCount(nEENumCmd->GetNewIntValue(newValue));
  }
  else if(command == phBinCmd)
  {
    run->SetLSPhotonCountBinWidth(phBinCmd->GetNewIntValue(newValue));
  }
  else if(command == phRefCmd)
  {
    run->SetLSPhotonCountLowerRef(phRefCmd->GetNewIntValue(newValue));
  }
  else if(command == phNumCmd)
  {
    run->SetLSPhotonCountBinCount(phNumCmd->GetNewIntValue(newValue));
  }
  else if(command == phCutCmd)
  {
    run->SetLSPhotonCountEventCut(phCutCmd->GetNewDoubleValue(newValue));
  }
  else if(command == phPMTCmd)
  {
    run->SetLSPhotonCountPMTSegregation(phPMTCmd->GetNewBoolValue(newValue));
  }
  else if(command == phTBinCmd)
  {
    run->SetPhotonTravelTimeBinWidth(phTBinCmd->GetNewDoubleValue(newValue));
  }
  else if(command == phTRefCmd)
  {
    run->SetPhotonTravelTimeLowerRef(phTRefCmd->GetNewDoubleValue(newValue));
  }
  else if(command == phTNumCmd)
  {
    run->SetPhotonTravelTimeBinCount(phTNumCmd->GetNewIntValue(newValue));
  }
  else if(command == phVBinCmd)
  {
    run->SetVetoPanelCountBinWidth(phVBinCmd->GetNewIntValue(newValue));
  }
  else if(command == phVRefCmd)
  {
    run->SetVetoPanelCountLowerRef(phVRefCmd->GetNewIntValue(newValue));
  }
  else if(command == phVNumCmd)
  {
    run->SetVetoPanelCountBinCount(phVNumCmd->GetNewIntValue(newValue));
  }
  else if(command == iEDBinCmd)
  {
    run->SetIonisationDepositBinWidth(iEDBinCmd->GetNewDoubleValue(newValue));
  }
  else if(command == iEDRefCmd)
  {
    run->SetIonisationDepositLowerRef(iEDRefCmd->GetNewDoubleValue(newValue));
  }
  else if(command == iEDNumCmd)
  {
    run->SetIonisationDepositBinCount(iEDNumCmd->GetNewIntValue(newValue));
  }
  else if(command == iEEBinCmd)
  {
    run->SetIonisationEscapeBinWidth(iEEBinCmd->GetNewDoubleValue(newValue));
  }
  else if(command == iEERefCmd)
  {
    run->SetIonisationEscapeLowerRef(iEERefCmd->GetNewDoubleValue(newValue));
  }
  else if(command == iEENumCmd)
  {
    run->SetIonisationEscapeBinCount(iEENumCmd->GetNewIntValue(newValue));
  }
  else if(command == iVDBinCmd)
  {
    run->SetVetoIonisationDepositBinWidth(iVDBinCmd->GetNewDoubleValue(newValue));
  }
  else if(command == iVDRefCmd)
  {
    run->SetVetoIonisationDepositLowerRef(iVDRefCmd->GetNewDoubleValue(newValue));
  }
  else if(command == iVDNumCmd)
  {
    run->SetVetoIonisationDepositBinCount(iVDNumCmd->GetNewIntValue(newValue));
  }
  else
  {
