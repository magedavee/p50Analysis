...
PrimaryGeneratorAction::PrimaryGeneratorAction()
{
  ...
  cryZoffset = detect->GetWorldSizeZ()/1000.0;
}
...
void PrimaryGeneratorAction::GenerateModuleParticleKinematics(G4int evtNo)
{
  ...
  switch(fModule)			// Switch structure to change models on-the-fly (in conjunction with Messenger class)
  {
   case InverseBeta:		// Inverse Beta Decay Module
   {
     if(evtNo == 0)
     {
       particle_source->SetCurrentSourceto(0);
   ...
   case FissionAntinu:		// Fission Antinu Module
   {
     if(evtNo == 0)
     {
       particle_source->SetCurrentSourceto(0);
   ...
   case CosmicNeutron:		// Cosmic Neutron Module
   {
     if(evtNo == 0)
     {
       particle_source->SetCurrentSourceto(0);
...
//    G4double theEnergy = neutron_generator->GenerateNeutronEnergyWithCDF();		// Quicker generation, but slightly less accurate
   ...
   case CosmicMuon:		// Cosmic Muon Module
   {
