...
double WilkinsonGamma(double Z = 1.) { return sqrt(1-(alpha_fs*Z)*(alpha_fs*Z)); }
...
double WilkinsonF_PowerSeries(double Z, double W, double R) {
    ...
    return 2.*(gm+1.)/(z*z)*pow(2*R,2*(gm-1))*sumCoeffs(coeffs,alpha_fs*Z,W/p,log(p));
}
...
double WilkinsonGammaMagSquaredApprox(double Z, double W, unsigned int N) {
    double s = 0;
    double gm = WilkinsonGamma(Z);
    double y = alpha_fs*Z*W/sqrt(W*W-1);
    double Ngm = N+gm;
    double a = (N+1.)/Ngm;
    double y1 = a*y;
...
double WilkinsonF0(double Z, double W, double R, unsigned int N) {
    ...
    double p = sqrt(W*W-1.);
    double F0 = 4.*pow(2.*p*R,2.*gm-2.)*GMi*GMi*exp(M_PI*Z*alpha_fs*W/p)*WilkinsonGammaMagSquaredApprox(Z,W,N);
    return F0<1e3?F0:0;
}
...
double WilkinsonL0(double Z, double W, double R) {
    ...
    if(!aiZ.count(Z)) {
        ...
        for(unsigned int i=0; i<6; i++)
            aiZi.push_back(coeff1(i,sumCoeffs(ai[i],alpha_fs*Z)));
        aiZ.insert(std::make_pair(Z,aiZi));
        aminus1Z.insert(std::make_pair(Z,sumCoeffs(aminus1,alpha_fs*Z)));
    }
    ...
    double gm = WilkinsonGamma(Z);
    double L0 = (1.+13.*(alpha_fs*Z)*(alpha_fs*Z)/60.-W*R*alpha_fs*Z*(41.-26.*gm)/(15.*(2.*gm-1.))
    -alpha_fs*Z*R*gm*(17.-2.*gm)/(30.*W*(2.*gm-1.))
    +aminus1Z[Z]*R/W+sumCoeffs(aiZ[Z],W*R)
    +0.41*(R-0.0164)*pow(alpha_fs*Z,4.5) );
...
double WilkinsonVC(double Z, double W, double W0, double R) {
    double gm = WilkinsonGamma(Z);
    return (1.-233.*alpha_fs*Z*alpha_fs*Z/630.-W0*R*W0*R/5.-6*W0*R*alpha_fs*Z/35.
    +(-13.*R*alpha_fs*Z/35.+4.*W0*R*R/15.)*W
    +(2.*gm*W0*R*R/15.+gm*R*alpha_fs*Z/70.)/W
    -4*R*R/15.*W*W);
}

double WilkinsonAC(double Z, double W, double W0, double R) {
    return (1.-233.*alpha_fs*Z*alpha_fs*Z/630.-W0*R*W0*R/5.+2*W0*R*alpha_fs*Z/35.
    +(-21.*R*alpha_fs*Z/35.+4.*W0*R*R/9.)*W
    -4.*R*R/9.*W*W);
}
...
double WilkinsonQ(double, double W, double W0, double M) {
    ...
    return 1.-M_PI*alpha_fs/(M*sqrt(W*W-1))*(1+B*(W0-W)/(3.*W));
}
...
double Wilkinson_g_a2pi(double W, double W0, double M) {
    ...
    )*alpha_fs/(2.*M_PI)+pow((W0-W),2*alpha_fs/M_PI*(athb/b-1.))-1.;
    return g==g?g:0;
}
...
double Davidson_C1T(double W, double W0, double Z, double R) {
    double p = sqrt(W*W-1);
    double y = alpha_fs*Z*W/p;
    double a2Z2 = alpha_fs*alpha_fs*Z*Z;
...
double Behrens_l2(double W, double, double Z, double R) {
    double p = sqrt(W*W-1);
    double y = alpha_fs*Z*W/p;
    double a2Z2 = alpha_fs*alpha_fs*Z*Z;
    double S0 = sqrt(1-a2Z2);
...
double shann_h_minus_g_a2pi(double W, double W0) {
    ...
    +athb/b*(2.-2.*b*b) - (W0-W)*(W0-W)/(6.*W*W) )*alpha_fs/(2.*M_PI);
}
