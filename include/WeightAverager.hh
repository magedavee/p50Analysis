...
public:
    /// Constructor
    WeightAverager(): sum_w(0) { for(uint i=0; i<N; i++) sum_wx[i] = sum_wxx[i] = 0; }
    /// Destructor
    ...
    /// get averaged value
    double get_avg(uint i) const { assert(i<N); return sum_wx[i]/sum_w; }
    /// get RMS
    double get_rms(uint i) const { assert(i<N); double u = sum_wxx[i]*sum_w - sum_wx[i]*sum_wx[i]; return (u>0 && sum_w>0)?sqrt(u)/sum_w:0; }
